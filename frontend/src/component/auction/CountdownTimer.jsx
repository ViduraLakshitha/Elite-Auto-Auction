...
const CountdownTimer = ({ endTime, onFinish, className }) => {
    const endTimee = new Date(endTime).getTime() - (5.5 * 60 * 60 * 1000);
    const [remainingTime, setRemainingTime] = useState(0);
    const intervalRef = useRef(); // Using useRef to store interval ID
    ...
    const formatTime = (milliseconds) => {
        ...
        if (totalSeconds >= 86400) {
            ...
            return (
                <p>
                    Ends In <span className='text-red-700 font-bold'>{hours.toString().padStart(2, '0')}</span> Hrs, 
                    <span className='text-red-700 font-bold'>{minutes.toString().padStart(2, '0')}</span> Min, 
                    <span className='text-red-700 font-bold'>{seconds.toString().padStart(2, '0')}</span> Secs
                </p>
            );
        }
    };
